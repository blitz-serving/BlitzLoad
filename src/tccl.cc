...
void mpi_initialize_tccl_context(uint32_t world_size,
  ...
  spdlog::info("Rank<{}> use RNIC {}", world_rank, name);
  ...
  for (uint32_t i = 0; i < world_size - 1; ++i) {
    sender_qp_list.push_back(rdma_util::RcQueuePair::create(gProtectionDomain));
    recver_qp_list.push_back(rdma_util::RcQueuePair::create(gProtectionDomain));
  }
  ...
  sender_scatter_data.insert(sender_scatter_data.begin() + world_rank,
                             {0, 0, rdma_util::HandshakeData()});
  recver_scatter_data.insert(recver_scatter_data.begin() + world_rank,
                             {0, 0, rdma_util::HandshakeData()});

  assert((sender_scatter_data.size() == world_size &&
  ...
  for (uint32_t root = 0; root < world_size; ++root) {
    ...
    MPI_Scatter(sender_scatter_data.data(), sizeof(ExchangeData), MPI_BYTE,
                &sender_gather_data[root], sizeof(ExchangeData), MPI_BYTE, root,
                MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
  }
  ...
  for (uint32_t root = 0; root < world_size; ++root) {
    // root scatter designated rdma_util::HandshakeData
    MPI_Scatter(recver_scatter_data.data(), sizeof(ExchangeData), MPI_BYTE,
                &recver_gather_data[root], sizeof(ExchangeData), MPI_BYTE, root,
                MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
  }
  ...
  for (uint32_t i = 0; i < world_size - 1; ++i) {
    gPickleSenderList.push_back(
        pickle::PickleSender::create(std::move(sender_qp_list[i]), 256 * 1024));
  ...
  gThreadController.add_thread(std::move(polling_thread),
