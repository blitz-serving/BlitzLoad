...
using rank_t = int;
struct chain_t {
  explicit chain_t() = default;
};

struct tanz_t {
  explicit tanz_t() = default;
};
...
public:
  NcclBcastor(const rank_t &rank, const rank_t &device, rank_t machine_rank,
              rank_t machine_size)
      : rank(rank), device(device), machine_rank(machine_rank),
  ...
  void broadcast(std::vector<rank_t> &&item, chain_t) {
    assert((rank - item.front() < machine_size &&
            item.back() - rank < machine_size));
    work_item.nlane = 0;
    work_item.dst_ranks =
        std::vector<rank_t>{std::make_move_iterator(item.begin()),
...
private:
    ...
    while (!shutdown.load(std::memory_order_relaxed)) {
      ...
      if (work_item.nlane == 0) {
        if (rank == work_item.dst_ranks[0]) {
          // src : send only
          nccl_bcastor_edge_inner(-1, work_item.dst_ranks[1] - world_to_machine,
                                  chain);
        } else if (rank == work_item.dst_ranks.back()) {
          // far end of broadcast chain
          nccl_bcastor_edge_inner(
              *(work_item.dst_ranks.end() - 2) - world_to_machine, -1, chain);
        } else {
          // intermediate
          auto it = std::find(work_item.dst_ranks.begin(),
                              work_item.dst_ranks.end() - 1, rank);
          nccl_bcastor_edge_inner(*(it - 1) - world_to_machine,
                                  *(it + 1) - world_to_machine, chain);
        }
      } else {
        /**
         ...
         *  \remark dst_ranks {0, 1, 2, 3, 4, 5} => shuffled_ranks {5, 0, 2, 4,
         * 1, 3, 5, 0}
         */
        std::vector<rank_t> const dst_ranks = std::move(work_item.dst_ranks);
        std::vector<rank_t> wrapped_ranks = {dst_ranks.back() -
                                             world_to_machine};
        for (auto it = dst_ranks.cbegin(); it < dst_ranks.cend(); it += 1) {
          wrapped_ranks.push_back((*it) - world_to_machine);
        }
        wrapped_ranks.push_back(dst_ranks.front() - world_to_machine);
        auto it = std::find(wrapped_ranks.begin() + 1, wrapped_ranks.end() - 1,
                            machine_rank);
        // iterator does not exceed boundary
        nccl_bcastor_edge_inner(*(it - 1), *(it + 1), tanz,
      ...
      awaken.store(false);
      awaken.notify_one();
      spdlog::info("Rank<{}> Broadcast Src elaspse: {}ms, Bandwidth={}GBps",
                   rank, elapse,
                   (double)weight_segment_size_in_bytes * 1000 / 1024 / 1024 /
                       1024 / elapse);
    }
  }
  ...
  void nccl_bcastor_edge_inner(rank_t last, rank_t next, chain_t) {
    spdlog::warn("Rank<{}>:({}) last = {} next = {}", rank, machine_rank, last,
                 next);
    if (last == -1) {
      /// \brief src
      ...
      size_t sent_bytes = 0;
      size_t nbytes = 0;
      LOG_ASSERT(buf != nullptr, "Buffer should not be nullptr");
      while (sent_bytes < weight_segment_size_in_bytes) {
        nbytes = std::min(chunk_size_in_bytes,
  ...
  /// \note last & next are both machine ranks
  void nccl_bcastor_edge_inner(rank_t last, rank_t next, tanz_t,
    ...
    spdlog::warn("Rank<{}>:({}) (nccl) last = ({}) next = ({})", rank,
                 machine_rank, last, next);

    char *buf = weight_segment;
    size_t const remain_bytes =
        weight_segment_size_in_bytes % chunk_size_in_bytes;
    char *const buf_bound = buf + weight_segment_size_in_bytes - remain_bytes;
    LOG_ASSERT((buf != nullptr), "Buffer should not be nullptr");

    char *fst_buf = buf + lane_id * strip_chunk_size_in_bytes;
    char *snd_buf = fst_buf;
    wrap_around_sub(snd_buf, buf, strip_chunk_size_in_bytes,
                    chunk_size_in_bytes);
    ...
    // can be optimized
    while (buf < buf_bound) {
      /// \pre primary buffer points to newly received chunk from other rank
      /// (Nvl or Rdma)
      ...
      for (size_t e = 0; e < nedges; ++e) {
        ...
        ncclGroupStart();
        ncclSend(fst_buf, strip_chunk_size_in_bytes, ncclChar, next, comm,
                 fst_stream);
        ncclRecv(snd_buf, strip_chunk_size_in_bytes, ncclChar, last, comm,
                 snd_stream);
        ncclGroupEnd();
        /// \post buf is unchanged
        recv_data_in_bytes += strip_chunk_size_in_bytes;
        /// \pre fst_buf is sent; wrap_sub sent buffer
        wrap_around_sub(fst_buf, buf, 2 * strip_chunk_size_in_bytes,
                        chunk_size_in_bytes);
       ...
       *  \remark snd == fst -- ;; fst <- (fst --) -- ;; swap fst snd -> snd ==
       * fst --
       *
       *  \remark additional Group Op will make fst_buf to its origin location
       *  \remark can't simply add one, because of Fermat's little law, n maybe
       * even
       */
      wrap_around_sub(fst_buf, buf, strip_chunk_size_in_bytes,
                      chunk_size_in_bytes);
      wrap_around_sub(snd_buf, buf, strip_chunk_size_in_bytes,
                      chunk_size_in_bytes);
      LOG_ASSERT((fst_buf == last_fst and snd_buf == last_snd),
                 "fst_buf 0x{:x} != last_fst 0x{:x} or snd_buf 0x{:x} != "
                 "last_snd 0x{:x}",
                 fst_buf, last_fst, snd_buf, last_snd);
    ...
    LOG_ASSERT(
        ...
        "{} + {} != {}", recv_data_in_bytes, remain_bytes,
    ...
    if (not(recv_data_in_bytes == weight_segment_size_in_bytes)) {
      ...
      if (fst_buf == buf) {
        rdma_sem.wait();
        ncclSend(res_buf, strip_nbytes, ncclChar, last, comm, fst_stream);
      } else if (snd_buf == buf) {
        ncclRecv(res_buf, strip_nbytes, ncclChar, next, comm, fst_stream);
      } else {
        ncclRecv(res_buf, strip_nbytes, ncclChar, next, comm, fst_stream);
        ncclSend(res_buf, strip_nbytes, ncclChar, last, comm, fst_stream);
      }
    }
    LOG_ASSERT((rdma_sem._lock == 0), "Rdma_sem's count != 0");
  }

  template <typename T>
  typename std::enable_if<std::is_pointer<T>::value>::type
  wrap_around_sub(T &self, T const &base, const size_t sub,
                  const size_t wrap) const noexcept {
    uintptr_t a = reinterpret_cast<uintptr_t>(self);
    uintptr_t b = reinterpret_cast<uintptr_t>(base);
    LOG_ASSERT((a >= b), "a 0x{:x} < b 0x{:x}", a, b);
    uintptr_t res = ((a - b) < sub) ? (a + wrap - sub) : (a - sub);
    LOG_ASSERT((res >= b && res <= b + wrap),
               "res {:x} < b {:x} of res {:x} > b + wrap {:x}", res, b, res,
               b + wrap);
    self = (T)res;
  }
