  ...
  for (const auto &mr : gMemoryRegionList) {
    if (addr >= uint64_t(mr->get_addr()) &&
      ...
      if (addr + length > uint64_t(mr->get_addr()) + mr->get_length()) {
        spdlog::error(
            "lkey [{}:{}] exceeds MR [{}:{}]", fmt::ptr((void *)addr),
            fmt::ptr((void *)(addr + length)),
            fmt::ptr((void *)mr->get_addr()),
            fmt::ptr((void *)((char *)mr->get_addr() + mr->get_length())));
      }
      assert((addr + length <= uint64_t(mr->get_addr()) + mr->get_length()));
      return mr->get_lkey();
    }
  }
  spdlog::error("Invalid address {}", fmt::ptr((char *)addr));
...
inline const char *rank_to_rnic_name(int rank) {
  ...
  std::string full_env =
      base_env + std::to_string(cuda_decives[rank % cuda_decives.size()]);
  const char *rnic_name = std::getenv(full_env.c_str());

  if (rnic_name == nullptr) {
    spdlog::error("No rnic found for rank {}", rank);
    throw std::runtime_error("No matching rnic for rank");
  }
  return rnic_name;
}

void mpi_initialize_tccl_context(uint32_t world_size,
                                 uint32_t world_rank) noexcept(false);

void bind_node(int node_id);

void register_mr(void *buffer, uint64_t length) noexcept(false);

inline pickle::Handle TcclSend(void *buffer, uint64_t length, uint32_t send_to,
                               uint32_t stream_id) noexcept(false) {
  assert(send_to < gWorldSize && send_to != gWorldRank);
  return gPickleSenderList[send_to > gWorldRank ? send_to - 1 : send_to]->send(
      stream_id, uint64_t(buffer), length, lkey(uint64_t(buffer), length));
}

inline pickle::Handle TcclRecv(void *buffer, uint64_t length,
                               uint32_t recv_from,
                               uint32_t stream_id) noexcept(false) {
  assert(recv_from < gWorldSize && recv_from != gWorldRank);
  return gPickleRecverList[recv_from > gWorldRank ? recv_from - 1 : recv_from]
      ->recv(stream_id, uint64_t(buffer), length,
             rkey(uint64_t(buffer), length));
}

inline void TcclChainMulticast(void *buffer, size_t length, uint32_t stream_id,
                               const std::vector<int> &ranks_in_chain,
                               size_t chain_index,
                               size_t chunk_size = 0x100000) noexcept(false) {
  // spdlog::debug("Rank<{}> Multicast group: [{}]", gWorldRank,
  ...
  if (chain_index == 0) {
    ...
    for (uint64_t i = 0; i < chunks; ++i) {
      ...
      TcclSend((char *)buffer + i * chunk_size, size,
               ranks_in_chain[chain_index + 1], stream_id)
          .wait();
      spdlog::debug("Rank<{}> sent {} bytes to Rank<{}>",
               ranks_in_chain[chain_index], size,
    ...
    for (uint64_t i = 0; i < chunks; ++i) {
      uint64_t size = std::min(length - i * chunk_size, chunk_size);
      handles.push_back(TcclRecv((char *)buffer + i * chunk_size, size,
                                 ranks_in_chain[chain_index - 1], stream_id));
    }
    for (uint64_t i = 0; i < chunks; ++i) {
      handles[i].wait();
      spdlog::debug("Rank<{}> received {} bytes from Rank<{}>",
    ...
    for (uint64_t i = 0; i < chunks; ++i) {
      uint64_t size = std::min(length - i * chunk_size, chunk_size);
      handles[i].wait();
      handle = TcclSend((char *)buffer + i * chunk_size, size,
                        ranks_in_chain[chain_index + 1], stream_id);
      spdlog::debug("Rank<{}> forwarded {} bytes to Rank<{}>",
               ranks_in_chain[chain_index], size,
               ranks_in_chain[chain_index + 1]);
    }
  }
...
[[deprecated]]
inline pickle::Handle TcclSend(uint64_t addr, uint64_t length, uint32_t send_to,
                               uint32_t stream_id) noexcept(false) {
  assert(send_to < gWorldSize && send_to != gWorldRank);
  return gPickleSenderList[send_to > gWorldRank ? send_to - 1 : send_to]->send(
      stream_id, addr, length, lkey(addr, length));
}

[[deprecated]]
inline pickle::Handle TcclRecv(uint64_t addr, uint64_t length,
                               uint32_t recv_from,
                               uint32_t stream_id) noexcept(false) {
  assert(recv_from < gWorldSize && recv_from != gWorldRank);
  return gPickleRecverList[recv_from > gWorldRank ? recv_from - 1 : recv_from]
